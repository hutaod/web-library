...
const components = ComponentConfig["menus"]["/components"] as [
  {
    title: string;
    children: {
      title: string;
...
components?.forEach((group) => {
  group.children?.forEach((item) => {
    ...
    componentToDemoPaths[item.title] = demos.filter((val) => val.startsWith(`${key}-demo`));
  });
});
    ...
    <div style={{ height: window.innerHeight }} className={styles.gallery}>
      ...
      <div className={styles.body} hidden={currentDemoIndex !== null}>
        <div className={styles.guide}>
          ...
          />
          <p>下面是一些 Aku Design Mobile 的组件 demo，可以点进去试一试</p>
          <p>
