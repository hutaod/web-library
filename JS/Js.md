...
`Symbol.hasInstance`：用于判断某对象是否为某构造器的实例。用于自定义 `instanceof` 操作符在某个类上的行为。
```
class MyArray {  
...
对象转换为原始类型时，先调用 `valueof()` 再调用 `toString()`，如果两者都没有返回原始类型，就会报错。
如果重写了`Symbol.toPrimitive`，就会直接调用该方法。

    Symbol.toPrimitive指将被调用的指定函数值的属性转换为相对应的原始值。
```
let a = {
  valueOf() {
    return 0
