...
| 算术右移 | a >> b | 把a的二进制表示向右移动b位，丢弃被移出的所有位.<br>(译注:算术右移左边空出的位是根据最高位是0和1来进行填充的) |
...
## JS进制转换
#### 十进制转其他进制
x.toString(n)可用于十进制转其他进制, n的有效值范围在2~36，最低为2，最高36，否则报错。
```
var x = 111;
...
#### 其他进制转十进制
parseInt(x,n)可用于其他进制转十进制，n的有效值范围在2~36，最低为2，最高36，当x正常时，n不在有效值范围，结果为NaN。
```
var x = "111";
...
#### 其他进制转其他进制
parseInt(x,n).toString(m)可用于其他进制转其他进制，n为x的进制，m为目标进制
```
